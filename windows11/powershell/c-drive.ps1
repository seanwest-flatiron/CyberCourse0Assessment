...
$secretFolderPath = Join-Path -Path $HOME -ChildPath "SECRET"
...
$flagFilePath = Join-Path -Path $secretFolderPath -ChildPath "FLAG-C.txt"
...
Write-Host "The flag is in a text file in the folder $secretFolderPath. Open that file in order to retrieve the flag."
